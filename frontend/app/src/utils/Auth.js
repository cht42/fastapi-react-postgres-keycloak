...
export class Auth {
  ...
  onAuthUpdateCallback() {
    for (var callback of this.onAuthUpdateCallbacks) callback();
  }
  ...
  isUserAuthenticated() {
    ...
    if (cookie && "authenticated" in cookie) return cookie.authenticated;
    else return false;
  }
  ...
  updateUserProfile = async () => {
    const user_profile_query = await {
      first_name: "cyprien",
      last_name: "huet",
      username: "chuet",
    };
  ...
  logoutUser = async () => {
    ...
    if (user_cookie) {
      if ("token" in user_cookie) {
        const logout_query = await this.#requestUserLogout(
          user_cookie["token"]
        );
